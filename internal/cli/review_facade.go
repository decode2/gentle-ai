...
func RunReview(args []string, stdout io.Writer) error {
	if len(args) == 0 || args[0] == "help" || args[0] == "-h" || args[0] == "--help" {
		_, _ = fmt.Fprintln(stdout, "Usage: gentle-ai review <capabilities|start|finalize|validate|status|invalidate|abandon|recover|reclaim|reconcile-authority|dispose-result|quarantine-legacy|repair-legacy-alias|schema|bind-sdd|maintenance> [flags]\n\nOrdinary review facade; repository scope, authority, canonical artifacts, and lifecycle transitions are derived by Go.")
...
func runReviewCommand(args []string, stdout io.Writer) error {
	...
	case "maintenance":
		return RunReviewMaintenance(args[1:], stdout)
	default:
...
type ReviewMaintenanceResult struct {
	Operation        string `json:"operation"`
	Action           string `json:"action"`
	StaleLockRemoved bool   `json:"stale_lock_removed,omitempty"`
}

func RunReviewMaintenance(args []string, stdout io.Writer) error {
	flags := newReviewFlagSet("review maintenance", stdout, "Perform review store maintenance.")
	cwd := flags.String("cwd", ".", "repository path")
	recoverStaleLock := flags.Bool("recover-stale-lock", false, "recover stale store locks from dead processes")
	if err := parseReviewFlags(flags, args); err != nil {
		return err
	}
	if reviewHelpRequested(args) {
	...
	if !*recoverStaleLock {
		return errors.New("review maintenance requires a specific maintenance action flag, e.g., --recover-stale-lock")
	}
	...
	if *recoverStaleLock {
		err := reviewtransaction.RecoverStaleLock(context.Background(), root)
		if err != nil {
			return fmt.Errorf("recover stale lock: %w", err)
		}
		return encodeReviewJSON(stdout, ReviewMaintenanceResult{Operation: "review/maintenance", Action: "recover-stale-lock", StaleLockRemoved: true})
	}
	return nil
}
