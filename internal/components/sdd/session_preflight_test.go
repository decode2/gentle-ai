...
func TestPreservedOpenCodeSessionPreflightPreservesLFAndCRLFBytes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "utf8 insertion", input: "前置🙂\n" + testSDDSessionPreflightInitAnchor + "\n后置🦊", want: "前置🙂\n" + block + "\n" + testSDDSessionPreflightInitAnchor + "\n后置🦊"},
		{name: "crlf insertion", input: "前置🙂\r\n" + testSDDSessionPreflightInitAnchor + "\r\n后置🦊", want: "前置🙂\r\n" + blockCRLF + "\r\n" + testSDDSessionPreflightInitAnchor + "\r\n后置🦊"},
		{name: "replace stale owned range", input: "保留前\r\n" + staleBlockCRLF + "\r\n" + testSDDSessionPreflightInitAnchor + "\r\n保留後", want: "保留前\r\n" + blockCRLF + "\r\n" + testSDDSessionPreflightInitAnchor + "\r\n保留後"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := migratePreservedOpenCodeSessionPreflight(tt.input)
			if err != nil {
