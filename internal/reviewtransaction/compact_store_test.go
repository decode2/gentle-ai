...
func TestCompactActualCumulativeOverflowRefusesWithoutMutation(t *testing.T) {
	repo := initSnapshotRepo(t)
	state, fix := pendingCompactCorrection(t, repo, "compact-cumulative-overflow")
	before, actual := state, state.CorrectionBudget+1
	err := state.CompleteCorrection(fix, actual, bindTargetedValidationForTest(ScopedValidationResult{
		LedgerIDs: state.FixFindingIDs, FixCausedFindings: []Finding{}, FollowUps: []FollowUp{},
		OriginalCriteria:     ValidationCheck{EvidenceHash: hash("2"), FixDeltaHash: FixDeltaHashForSnapshot(fix), Passed: true},
		CorrectionRegression: ValidationCheck{EvidenceHash: hash("3"), FixDeltaHash: FixDeltaHashForSnapshot(fix), Passed: true},
	}, fix))
	...
	if !reflect.DeepEqual(state, before) || state.CumulativeCorrectionLines > state.CorrectionBudget ||
		!equalStrings(state.CorrectionScopePaths(), before.CorrectionScopePaths()) {
		t.Fatalf("over-budget actual mutated authoritative accounting or scope: %#v", state)
	}
}
